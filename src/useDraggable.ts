...
import { getCurrentInstance, isRef, nextTick, onMounted, onUnmounted, unref, watch, type Ref } from 'vue';
import {
  extend,
  forEachObject,
...
export interface UseDraggableReturn extends Pick<Sortable, SortableMethod> {
   ...
   * @default By default the root element of the VueDraggable instance is used
   */
...
export function useDraggable<T>(el: null | undefined, options?: RefOrValue<UseDraggableOptions<T>>): UseDraggableReturn;
...
export function useDraggable<T>(...args: any[]): UseDraggableReturn {
  ...
  const { immediate = true, clone = defaultClone, customUpdate } = unref(options) ?? {};
  ...
  const methods = {
    ...
    closest: (...args) => instance?.closest(...args) ?? null,
  } satisfies Pick<Sortable, SortableMethod>;
