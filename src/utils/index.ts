 ...
 * @returns {Element}
 */
export function getElementBySelector(selector: string, parentElement: Document | Element = document) {
 ...
 * @param {any} [ctx] - The context of the function.
 * @returns {Function}
 */
export function mergeExecuted<T extends (...args: []) => any>(fn: T, afterFn: T, ctx: any = null) {
 ...
 * @returns {Record<string, any>}
 */
export function mergeOptionsEvents(options: Record<string, any>, events: Record<string, any>) {
  const evts = { ...options };
 ...
 * @param fn
 */
export function forEachObject<T extends Record<string, any>>(obj: T, fn: (key: keyof T, value: T[keyof T]) => void) {
